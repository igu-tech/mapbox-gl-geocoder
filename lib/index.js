...
MapboxGeocoder.prototype = {
  ...
  onAdd: function(map) {
    ...
    if (this.options.searchTypes) {
      this.searchTypes = this.options.searchTypes;
    } else {
  ...
  _geocode: function(searchInput) {
    ...
    // mecklenburg request, yo
    var request = fetch(
      "https://mcmap.org/api/search/v1/" +
        searchInput.toLowerCase() +
        "?tables=" +
        _this.searchTypes.join(",")
    )
      ...
      .then(function(data) {
        data.forEach(function(element) {
          geocodes.push({
            center: [Number(element.lng), Number(element.lat)],
            geometry: {
              type: "Point",
              coordinates: [Number(element.lng), Number(element.lat)]
            },
            place_name: element.label,
            place_type: [element.type],
            properties: {},
        ...
        res.features = geocodes;

        // apply results filter if provided
        if (_this.options.filter && res.features.length) {
          res.features = res.features.filter(_this.options.filter);
        }

        if (res.features.length) {
          _this._clearEl.style.display = "block";
        } else {
          _this._clearEl.style.display = "none";
          _this._typeahead.selected = null;
        }

        _this._eventEmitter.emit("results", res);
        _this._typeahead.update(res.features);
        _this._loadingEl.style.display = "none";
      })
      .catch(function(ex) {
        console.log("geocode failed failed", ex);
      });
